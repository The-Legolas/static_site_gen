...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path, max_depth=20, depth=0, visited=None):
    if visited is None:
        visited = set()
    if depth > max_depth:
        ...
        for entry in os.listdir(dir_path_content):
            ...
            try:
                if os.path.isfile(dir_full_path) and dir_full_path.endswith(".md"):
                    ...
                    generate_page(dir_full_path, template_path, dest_full_path, base_dir=dir_path_content)
                if os.path.isdir(dir_full_path):
                    os.makedirs(dest_full_path, exist_ok=True)
                    generate_pages_recursive(dir_full_path, template_path, dest_full_path, max_depth, depth + 1, visited)
